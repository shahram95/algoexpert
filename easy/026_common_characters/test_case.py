import json
from main import commonCharacters
...
for i in range(1,total_test_cases+1):
	...
	output = commonCharacters(test_case["strings"])
	print("Your solution is: {}".format(output))
	print("The ground truth is: {}".format(test_case["output"]))
	
	
	total_test_cases_passed += 1 if sorted(output) == sorted(test_case["output"]) else 0
	pf_str = "Passed" if sorted(output) == sorted(test_case["output"]) else "Failed"
